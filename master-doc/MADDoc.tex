...
\chapter{Vorwort}
Die Welt befindet sich momentan im großen Wandel: Der Fortschritt der Technologie lässt nicht nach, im Gegenteil, er steigt mit exponentieller Geschwindigkeit an und ein Ende ist nicht in Sicht. Das Internet beschränkt sich inzwischen nicht mehr nur auf das Geschäftswesen oder gar auf Universitäten, sondern hält Einzug in Bereiche, die man sich bei der Entstehung desselben nie ausgemalt hätte. Selbst Haushaltsgeräte oder Autos können inzwischen über das Internet kommunizieren. Es ist inzwischen unmöglich geworden, den technischen Stand, welcher in einigen Jahren herrschen wird, präzise  vorauszusagen.\\
...
Als Nächstes war es an uns, einen geeigneten Betreuer zu finden. Trotz des großen Spektrums an verschiedensten Lehrern fiel die Entscheidung schlussendlich auf Prof. Dr. Weiss, welchen wir bereits im Jahr zuvor kennenlernen durften.\\
Seine Gelassenheit und große Kompetenz, die er in Fächern wie Informatik und vor allem Netzwerklabor zeigte, beeindruckten uns.\\

Als wir ihm bei einem ersten Gespräch unsere bisherigen Gedankengänge aufzeigten, entwickelte er mit uns zusammen schließlich die Idee, unsere Interessen zu kombinieren und ein Programm zu entwickeln, dessen Aufgabe darin besteht, ein Netzwerk zu überwachen.
\end{onehalfspace}
\begin{singlespace}
...
\begin{onehalfspace}
\chapter{Abstract}
Mit den immer größer werdenden Anforderungen an die Netzwerktechnik, ist es zur Herausforderung geworden, alles im Blick zu behalten. Große Firmen wie z.B. Cisco und HP bieten für dieses Problem Auswege. Es gibt einige kostenlose Produkte, die auch dabei helfen ein Netzwerk zu überwachen, wie z.B. Nagios.
\paragraph{Problemstellung/Ziel:} Die oben genannten Produkte sind jedoch sehr umfassend und komplex. Diese Diplomarbeit hatte das Ziel eine kompakte Software zu programmieren, die sowohl ein Heimnetzwerk als auch ein kleines Firmennetzwerk (max. 50 Netzwerkgeräte) überwachen kann.
\paragraph{Methodik:} Das Produkt Nagios war uns eines erstes Vorbild. Unser Ansatz bestand darin, dass nur grundlegende Funktionen zur Netzwerküberwachung verwendet wurden. Detaillierte Planung war ein weitere sehr ernstzunehmender Aspekt. Die Wahl der Programmiersprache, fiel auf C\#.. Als die Programmziele festgelegt waren, teilten wir die Aufgabe in mehrere Teilbereiche auf. Als jeder seine Teilbereiche wusste, programmierte er für sich. Durch das Nutzen von Git, einem Versionsverwaltungsprogramm, war eine weitgehend reibungslose Zusammenarbeit möglich.
\paragraph{Resultat:} Das Ergebnis war ein Programm mit sowohl einem CLI (text basierte Schnittstelle) als auch einem GUI (graphische Benutzeroberfläche). Das MAD - Networkmonitoring bietet sich an als grundlegende und kompakte Netzwerküberwachung. Mit Funktionen wie beispielsweise einem Portscanner oder Dienstüberprüfungen wie z.B. dem HTTP-checker.
...
Für die Benutzung des Programmes soll eine textbasierende Schnittstelle implementiert werden, die als primäre Interaktion mit dem Programm dient. Die vollständige Überwachung aller Aufgaben und Geräte soll über diese Schnittstelle möglich sein.\\
Es soll auch die Möglichkeit bestehen, die textbasierende Schnittstelle über das Netzwerk zu nutzen. Daher muss diese Schnittstelle als Netzwerkservice implementiert werden und es wird eine Client-Software benötigt.\\
...
Die Resulate der Überwachungen sollen in einer Datenbank lokal abgespeichert werden. Eine Möglichkeit die Datensätze der Datenbank zusammenzufassen, soll implementiert werden, um den Speicherbedarf zu verringern.\\
...
\subsubsection{Heute}
E-Mails sind heute einer der meistgenutzten Kommunikationsmöglichkeiten überhaupt. Vor allem Firmen senden massenhaft E-Mails. Die elektrische Post ist aus dem heutigen Alltag kaum mehr wegzudenken.\\\\
...
\section{Probleme}
Beim Versand von E-Mails gibt es nach wie vor auch einige Probleme, die schwer zu bewältigen sind. 
...
Das Transaktionkonzept gehört auch zu den wichtigsten Aspekten der Datensicherheit. Dieses soll sogenannte "{}Race Conditions"{} verhindern, welche beim nicht autorisierten parallelen Aufruf von Daten aus der Datenbank entstehen können. Daten könnten gleichzeitig von mehreren Benutzern verändert werden, dies würde zu inkonsisten Daten führen und Änderung würden vom Zufallen abhängig sein. Die Lösung ist es nur einem Benutzer die Berechtigung zu erteilen Änderungen an den Daten vornehmen zu können, währen andere gesperrt bleiben.\\
%http://de.wikipedia.org/wiki/Datenbank#Transaktionen

Das Sicherstellen der Integrität erfolgt durch "{}Constraints"{}. Es handelt sich um Regeln im Managementsystem, die zur Beschreibung der Datenveränderungen dienen. Foreign Key Constraint ist der wichtigste bei relationalen Datenbanksystemen. Durch diesen werden Daten, die von anderen Tabellen benötigt werden nicht gelöscht. Andere Bedingungen regeln das Duplizieren, einzelner Inhalte bestimmter Datenfelder, ...
...
Dadurch hat SQLite eine Anbindung an viele Programmiersprachen.
...
Die Basis der Syntax und Semantik holte sich Hipp aus der PstgreSQL 6.5 Dokumentation. Im August 2000 wurde SQLite Version 1.0 vorgestellt. Die Speicherung wurde damals auf Basis von gdbm realisiert (GNU Database Manager). SQLite 2.0 ersetzte gdbm mit einer B-tree Implementierung, durch welche Transaktionen ermöglicht waren. SQLite 3.0 wurde zum Teil von America Online entwickelt und weiter ergänzt. \\
...
SQLite hat viele wichtige Eigenschaften, zu den wichtigsten Eigenschaften zählen:
\begin{itemize}
...
\item gesamte Datenbank ist in eine Datei gespeichert ("{}single disk file"{}), optimal für den Einsatz in Anwendungen
...
\item erhältlich als eine ANSI-C Quellcodedatei, welche einfach kompiliert werden kann und deswegen leicht in größere Projekte eingebunden werden kann
...
\item Quellen sind in der sogenannten "{}Public Domain"{}, diese erlaubt die Benutzung für sämliche Zwecke
\item usw.
\end{itemize}
...
Diese sind sehr einfach zu lesen und im Prinzip selbsterklärend. Es werden Befehle in der richtigen Reihenfolge aufgelistet und auch die verschiedenen dazugehörigen möglichen Parameter. Es gibt zu jeder möglichen Funktion ein entsprechendes Syntaxdiagramm. Die wichtigsten und grundlegensten Diagramme, welche zur Realisierung des Datenbankmanagementsystems nötig waren, werden im Folgenden aufgelistet. Alle möglichen Befehle und Befehlkombinationen sind auf der Homepage von SQLite (www.sqlite.org) abrufbar.
...
Eine \textbf{Hashfunktion} ist eine Funktion, die eine beliebige Menge an Eingangsinformationen auf eine fixe Länge an Ausgangsinformationen abbildet.  Das Ziel von Hashfunktionen ist das Erschweren der Suche nach Kollisionen. Kollisionen können bei Hashfunktionen nicht verhindert werden, da die Eingangsgröße keine maximale Länge hat. Ein weiteres Ziel von Hashfunktionen ist das Verhindern der Möglichkeit des Rückrechnens, also die Ermittlung der Eingangsgröße aus der Ausgangsgröße.\cite{wiki04}
...
Das oberste Ziel der Kryptoanalyse ist die Ermittlung des Schlüssels, um an den Wert der Informationen zu gelangen. Damit die Kryptoanalyse zum Ziel kommen kann, muss das kryptographische System ausgehebelt werden, entweder durch ausprobieren des Schlüssels oder anderen Methoden. Oftmals ist das Knacken des kryptographischen Systems für die Ermittlung des Schlüssels gar nicht notwendig. Die falsche Anwendung des kryptographischen Systems oder schlecht gewählte Passwörter reichen meistens aus, um das kryptographische System zu umgehen.
...
Nun können für jede Netzwerkaufgabe Regeln definiert werden, wann eine Benachrichtigung notwendig ist z.B. kann eine Regel definiert werden, die besagt, dass falls eine Netzwerkaufgabe mehr als 2000ms für die Ausführung benötigt, eine Benachrichtung gesendet werden soll.

\section{Netzwerkknoten}

Ein Netzwerkknoten stellt ein Netzwerkgerät dar. Diese Gerät wird durch die folgenden Eigenschaften charakterisiert:
...
Jobs definieren bestimmte Aufgaben, die im Netwerk erledigt werden sollen. Das Ziel der Aufgabe ist immer die IP-Adresse des Netzwerkknotens, an der der Job drangehängt wurde. Erst wenn die definierte Zeit erreicht wurde, wird der Job vom JobSystem in einem eigenen Thread ausgeführt. Nachdem der Job die Aufgabe erledigt hat, überprüft er selbstständig ob er die definierten Regeln gebrochen hat und schreibt die Resultate der Aussführung in die Datenbank. Welche Operationen der Job beim Start ausführt, hängt vom Job-Typ ab. Die Resultate, die ein Job erbringen kann, hängt vom Typ ab.
...
\subsubsection{Check Jobs (Daniel Ranalter)}
