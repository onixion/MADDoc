...
\end{scriptsize}
\end{center}
Die einzelnen Schritte in der obigen Abbildung:
\begin{enumerate}
...
Die einzelnen Schritte in der obigen Abbildung:
\begin{enumerate}
\item \textbf{Server:} Server hört auf den Port und wartet auf eventuelle Anfragen\\\\
\textbf{Client:} Client sendet eine Anmeldungsanfrage
\item \textbf{Server:} Bestätigt Anmeldung\\\\
\textbf{Client:} Wählt einen Ordner aus (inbox)
\item \textbf{Server:} Sendet Übersicht des Ordners\\\\
\textbf{Client:} Fordert Informationen über E-Mail Nr.12
\item \textbf{Server:} Sendet Informationen über E-Mail Nr.12\\\\
\textbf{Client:} Client fordert Header
\item \textbf{Server:} gewünschter Header wird gesendet\\\\
\textbf{Client:} Löscht E-Mail Nr.12
\item \textbf{Server:} Bestätigt den Löschvorgang\\\\
\textbf{Client:} Meldet sich ab
\item \textbf{Server:} Bestätigt Abmeldung
\end{enumerate}
...
\item 110/TCP
\item 995/TCP\\
für POPS (POPSecure unterscheidet sich insofern, dass anfänglich eine Verschlüsselung (siehe Kapitel \ref{chap:krypto} Seite \pageref{chap:krypto}) zwischen Client und Server ausgemacht wird.)

\end{itemize}
\item Netzwerklayer: Anwendung
\item Dokumentation: RFC 1939
\end{itemize}
Das Post Office Protocol ist eine weitere Möglichkeit um E-Mails vom Mailserver abzurufen. Auch hier gibt es mehrere Versionen, wobei POP3 die zu verwendende ist. Das POP3 erschien erstmals im Jahre 1988 und ist somit auch ein relativ altes, jedoch intaktes Protokoll.
\paragraph{Allgemeine Funktionsweise\\}
POP ermöglicht es die E-Mails vom Mailserver auf ein lokales Speichermedium zu laden und speichern. Damit können E-Mail lokal verwalten werden und es ist keine dauernde Verbindung zu Server notwendig.

Dieses Protokoll hat ebenfalls gewisse Vorteile, aber auch Nachteile. Diese werden in der Gegenüberstellung (siehe Kapitel \ref{sssec:IMAPvsPOP} Seite \pageref{sssec:IMAPvsPOP}) näher erläutert.\\\\
...
\begin{enumerate}
\item \textbf{Server:} Server hört auf den Port und wartet auf eventuelle Anfragen\\\\
\textbf{Client:} Client sendet eine Anfrage
\item \textbf{Server:} Bestätigt und zeigt somit, dass er aktiv ist\\\\
\textbf{Client:} Gibt Usernamen durch
\item \textbf{Server:} Verlangt nach Passwort\\\\
\textbf{Client:} Gibt Passwort durch
\item \textbf{Server:} Bestätigt Anmeldung\\\\
\textbf{Client:} Client fragt Status ab
\item \textbf{Server:} Sendet aktuellen Status\\\\
\textbf{Client:} Fordert Auflistung
\item \textbf{Server:} Listet E-Mail auf\\\\
\textbf{Client:} Fragt E-Mail Nr.1 ab
\item \textbf{Server:} Sendet E-Mail Nr. 1 komplett zu\\\\
\textbf{Client:} Löscht E-Mail Nr.1
\item \textbf{Server:} Bestätigt, dass die E-Mail für das Löschen Markiert wurde\\\\
\textbf{Client:} Meldet sich ab
\item \textbf{Server:} Löscht nun die E-Mail und bestätigt die Abmeldung\\\\
\end{enumerate}

\subsubsection{Gegenüberstellung IMAP - POP} \label{sssec:IMAPvsPOP}
Die beiden Protokolle IMAP und POP haben beide ihre Vor- und Nachteile. Beide werden heute sehr viel verwendet, wobei POP etwas verbreiteter ist.\\\\
\begin{center}
\begin{tabular}{p{6cm}|c|p{6cm}}
\textbf{IMAP} & vs. &\textbf{POP}\\
\hline
Alle Nachrichten werden auf dem Server gespeichert und nur sehr wenig lokal Speicher wird benötigt. & $\leftarrow$ & Abgerufen E-Mail werden nochmals auf lokalem Speicher gespeichert und sind somit doppelt vorhanden.\\
\hline
Die Zeit bis eine bestimmte E-Mail gelesen werden kann ist sehr kurz & $\leftarrow$ & Es dauert bei POP etwas länger da die ganze E-Mail zuerst heruntergeladen werden muss\\
\hline
Server und MUA (siehe Kapitel \ref{itm:MUA} Seite \pageref{itm:MUA}) sind immer synchron & $\leftarrow$ & Alle E-Mail werden von dem MUA lokal verwalten, dadurch kann es keine Synchronisation zwischen MUA und MDA (siehe Kapitel \ref{itm:MDA} Seite \pageref{itm:MDA}) geben.\\
\hline
Server werden stark belastet, da die ganzer Verwaltung Online vonstattengeht. & $\rightarrow$ & Da alles lokal verwaltet wird sind die Server nicht ausgelastet.\\
\hline
Dauernde Verbindung zum Server ist notwendig & $\rightarrow$ & Da die E-Mails lokal gespeichert werden ist die Verbindung zum Server nur während des Downloads notwendig.
\end{tabular}
\end{center}
\paragraph{Vergleich mit der Post:}
\subparagraph{IMAP:} Die Post liegt im Briefkasten. Man geht raus zum Briefkasten und ließt, sortiert und entsorgt jegliche Post im Briefkasten.
\subparagraph{POP:} Die Post liegt im Briefkasten. Man geht raus zum Briefkasten und kopiert die Post oder aber entsorgt sie am noch im Briefkasten. Man kann die Kopien mit nach Hause nehmen.
\section{Bedeutung im Altag}
\subsubsection{Heute}
E-Mails sind heute einer der meistgenutzten Kommunikationsmöglichkeiten überhaupt. Vor allem Firmen senden massenhaft E-Mails. D.h. sie sind aus dem heutigen Alltag nicht mehr wegzudenken.\\\\
Viele Geräte unterstützen diese Technik, wie z.B. Smartuhren, Smartphones, Tablets, Laptops, etc...\\
Dadurch wird auch noch ermöglicht E-Mails sehr schnell Abrufbar sind.
\paragraph{Nennenswerte E-Mail - Anbieter:}
\begin{itemize}
\item GMAIL
\item Outlook
\item GMX
...
Analysten zufolge wird der E-Mail Verkehr sich immer mehr in den Wirtschaftlichen Sektor verschieben, da die Menschen immer mehr ihre Sozialen Kontakte über Soziale Netwerke, wie Facebook, Whatsapp und Twitter aufrecht erhalten. Somit werden kurze E-Mails zwischen zwei Freunden immer seltener.
\section{Probleme}
Es gibt aber auch einige Probleme bei E-Mails.
\subsection{Begrenzungen}
E-Mails könnten zwar rein theoretisch unbegrenzte Datenmengen übertragen, dies ist jedoch in der Praxis nicht umsetzbar. Viele Mailserver-Betreiber limitieren die Datenmengen um die Server möglichst wenig auszulasten.
\subsection{Informations-Overload}
Jeder bekommt heute unzählige E-Mail. Diese variieren natürlich, manche sind Werbung, mache sind Wichtig und mache Gefährlich. Auf jeden Fall sind es sehr viele Mails und diese müssen tagtäglich durchforstet werden. Die führt dazu, dass man gezwungenermaßen mit Informationen überhäuft wird.
\subsection{Spam}\label{sssec:Spam}
Als Spam werden jene E-Mail bezeichnet, welche meist werbenden Inhalt haben. In 2010 z.B lag der Prozentsatz von eben solchen Spam-Mails auf rund 90\%, bei 107 Billionen versendeten E-Mails auf der ganzen Welt (vgl.http://de.wikipedia.org/wiki/E-Mail\#Spam) 
\chapter{Oberfläche}
lorem ipsum
...
\section{MAD-DB}
Die Realisierung der Datenbank erfolge über SQLite. SQLite verwendet ein relationales Datenbankmanagementsystem, welches in C\# durch das Einbinden der "{}sqlite3.dll"{} konfigurierbar ist. Es werden dadurch alle Befehle von SQLite verwendet werden und es sind keine weiteren Installationen notwendig. Die gesamte Datenbank wird anschließend als eine Datei gespeichert.
...
\begin{itemize}
\item Ping\\
Der Ping dient zur grundsätzlichsten Überprüfung eines Hosts. Er speichert die Zeit welche er benötigt hat (Roundtrip time), den Erfolgsstatus, die ICMP Antwort (oder IPStatus) und die verbleibende TTL in der Datenbank. Ping verwendet ICMP.\\
Die betroffenen Datenbankfelder wären also: GUID, JOBNAME, JOBTYPE, PROTOCOL, OUTSTATE, Start Time, Stop Time, Delay Time
\item HTTP\\
Über den Job HTTP wird die Verfügbarkeit eines WebServers überprüft. Es gibt nur einen Output, welcher angibt ob der Job erfolgreich war oder nicht. Der Job schickt eine Anfrage an die Zieladdresse an den Port 80. Kann der zurückgegebene header ausgelesen werden, wird der Job als erfolg gewertet. \\
Die betroffenen Datenbankfelder wären also: GUID, JOBNAME, JOBTYPE, PROTOCOL, OUTSTATE, Start Time, Stop Time, Delay Time
\item Portscan\\
Der Portscan überprüft einzelne Ports auf ihre Funktionstüchtigkeit indem er ein Paket an sie sendet und auf eine Antwort wartet. \\
Die betroffenen Datenbankfelder wären also: GUID, JOBNAME, JOBTYPE, PROTOCOL, OUTSTATE, Start Time, Stop Time, Delay Time
\item DNS\\
DNS überprüft ob der DNS Server funktionsfähig ist. Dies bewerktstelligt er, indem er versucht den eigenen Hostnamen aufzulösen. Bei diesem Job beschränkt sich der verwertbare Output auf ein einfaches funktioniert\/ funktioniert nicht. \\
Die betroffenen Datenbankfelder wären also: GUID, JOBNAME, JOBTYPE, PROTOCOL, OUTSTATE, Start Time, Stop Time, Delay Time
\item FTP\\
FTP überprüft ob der FTP Server funktionsfähig ist. Dies bewerkstelligt er, indem er versucht das momentane Verzeichnis des FTP Servers abzurufen. Der Output bei diesem Job liefert neben dem funktioniert\/ funktioniert nicht, auch eine nähere Beschreibung warum es nicht funktioniert. \\
Die betroffenen Datenbankfelder wären also: GUID, JOBNAME, JOBTYPE, PROTOCOL, OUTSTATE, Start Time, Stop Time, Delay Time
\item SNMP\\
SNMP überprüft ob der SNMP Server funktionsfähig ist. Dies ermittelt er indem er den Hostnamen aus den SNMP MIBs ausließt und diesen mit dem des Hosts überprüft. Stimmt dieser überein wird es als Erfolggewertet. Auch dieser Job speichert nur fuktioniert\/ funktioniert nicht. \\
Die betroffenen Datenbankfelder wären also: GUID, JOBNAME, JOBTYPE, PROTOCOL, OUTSTATE, Start Time, Stop Time, Delay Time,
\item Hosts\\
Die Hosts werden mittels DHCP und ARP aus dem Netzwerk gefischt. Da DHCP nur neu hinzukommende Hosts auffangen kann, wurde ARP implementiert, welches alle findet. Diese werden dann in die Datenbank gespeichert. Durch regelmäßiges anpingen der Hosts wird deren Aktivität überprüft. 
Auch die Möglichkeit Hosts manuell einzutragen wird gegeben sein. \\
Die betroffenen Datenbankfelder wären also: GUID, HOST, IP, MAC, OUTSTATE
\item Traffic\\
Eine weitere Funktionalität ermöglicht das Auslesen von ausgehendem Traffic eines Hosts via SNMP.  Auch dieser wird dann in der Tabelle abgespeichert.\\
Die betroffenen Datenbankfelder wären also: GUID, JOBNAME, JOBTYPE, PROTOCOL, Description, Start Time, Stop Time, Delay Time
\end{itemize}
...
13.04.2014 bis 14.04.2014 & 6h & CLI-Framework: Design - Überlegungen
