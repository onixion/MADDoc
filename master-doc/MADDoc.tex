...
Bei einer Programmiersprache handelt es sich um eine formale Sprache, mit welcher Datenstrukturen und Algorithmen formuliert bzw. erstellte werden, welche vom Computer ausführbar sind. Datenstrukturen und Algorithmen sind definierte Rechenvorschriften, welche sich durch Anweisungen nach bestimmten Mustern (Syntax) erstellen lassen.\\
...
z.B.: Bei der Deklaration von Varablen in den Programmiersprachen PHP und Java. In der Programmiersprache PHP wird mit einem Befehl (var) eine Variable deklariert, die Typenzuweisung erfolgt automatisch. Bei Java muss immer der Datentyp der Variable korrekt zugewiesen werden. [Was hat die Rechtschreibung bzw. Vokabular mit der Typenzuweisung zu tun?! Es ist ein unterschied, aber hat nichts mit der Lexik zu tun.]
\item Syntax\\
...
Von Programmiersprachen wird oft folgendes mindestens zur Verfügung gestellt: [vieleicht neu-formulieren]
\begin{itemize}
...
\\Von statischen Bibliotheken spricht man, wenn durch die Applikation der Umfang und der Speicherbedarf vergrößert werden.[neu-formulieren]
...
Bei Verwendung von statischen Bibliotheken, wäre die von mehreren Prozessen verlangte Funktion für jeden Prozess in den Arbeitspeicher geladen worden. [Was willst du uns sagen?]\\
...
\end{itemize}
Durch die Überschneidung von Compilersprachen und Skriptsprachen in den Anwendungsgebieten und ihren jeweiligen Eingenschaften, ist eine genauere Unterscheidung nur schwer ersichtlich.[nochmal genau durchschauen und vieleicht neu]
...
Es ist das älteste Programmierparadigma. Die Entstehung ist durch die Größe älterer Programmiersprachen zustandegekommen, klassische Programmierart. Die bildet die Basis für einige Programmiersprachen. [Check nicht was du sagen willst]
\begin{itemize}
\item ALGOL
\item Fortran
\item Pascal
...
\\Auch eine einfache Vererbung wird zur Verfügung gestellt. Durch die bereits vorhandene .NET Framework und Common Language Runtime werden Garbage Collection, Sprachinteroperabilität, weiterentwickelte Sicherheitskonzept und Versionskontrollen erweitert worden. ['erweitert worden' - häh?]\\
...
\begin{center}
%\includegraphics[scale=0.6]{img/uebersetzung.png}\\
...
Threads, welche zu einem Prozess gehören, werden von verschiedenen Prozessoren ausgeführt, wenn mehrere Prozessoren vorhanden sind, oder nehmen voneinander unabhängige Registersätze und Stacks, welche verschiedenen Teilen des Adressraums zugeordnet sind. [nicht verständlich]\\
...
Threads von Programmfunktionen sind übersichtlich aufteilbar, jeder für eine Aufgabe die Verantwortung trägt. [nicht verständlich] Je nach Betriebssystem können Threads verschiedene Zustände annehmen, bei dem Großteil sind es: 
\begin{itemize}
...
Beziehungen zwischen Tabellen. ("{}Buch"{})(Quelle: http://de.wikipedia.org/wiki/Relationale\_ Datenbank\# Beziehungen\_ zwischen\_ Tabellen)
\end{center}
...
Alle Tabellen bis auf die Tabellen JobTable und MemoTable bestehen aus zwei Datenfelden. Das erste Datenfeld ist immer eine ID, welche den primary key vom Typ Integer darstellt, das zweite ist immer ein Feld vom Typ Text, dessen Inhalt gleich dme jeweiligen Namen der Tabelle entspricht. 
