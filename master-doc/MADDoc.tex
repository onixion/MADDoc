...
CLI steht für \textbf{Command Line Interface} (=text-basierende Schnittstelle) und darunter versteht man Schnittstellen, die für die Interaktion mit Programmen zeilenweise Befehle in Form von Text einliest, interpretiert und anschließend ausführt. Programme, die diese Schnittstelle anbieten, werden \textbf{CLI-Interpreter} genannt. Der Begriff \textbf{Shell} hat sich unter unixähnlichen Betreibssystemen als eine andere Bezeichnung für einen CLI-Interpreter eingebürgert. Bekannte CLI-Interpreter sind zum Beispiel \textbf{sh}, \textbf{bash} oder \textbf{Windows PowerShell}. (vgl. http://de.wikipedia.org/wiki/Kommandozeile http://de.wikipedia.org/wiki/Unix-Shell)
