...
Die technische Realisierung besteht aus 2 Teilen:
\begin{itemize}
\item \textbf{Get Parameter - Zuweisen der Parameter}\\
In diesem Programmteil wird eine Methode (SetSendMail) geboten. Mit dieser Methode kann man die für eine E-Mail notwendigen Parameter (Betreff, Inhalt, Anzahl der Fehlversuche beim Senden) zuweisen. Falls aber noch andere Parameter gesetzt werden sollen, wie z.B. vom Standard abweichende Empfänger oder Sender Adresse, uvm..
Nachdem alle Parameter übergeben worden sind wird ein sogenanntes NotficPackIn erstellt, dies ist ein Paket mit allen möglichen Parametern.\\
Immer wenn ein solches Paket zusammengestellt wird, wird es der Warteschlange hinzugefügt, wo es dann darauf wartet, verarbeitet zu werden.\\
Anschließend wird, falls nicht bereits vorhanden, ein Thread gestartet, welcher die Warteschlange abarbeitet. Der Warteschlange können laufend Pakete hinzugefügt werden, der Thread stoppt erst wieder wenn die Warteschlange leer ist.\\
...
\end{lstlisting}
\item \textbf{Recive Package \& Send - Der eigentliche Sendevorgang}
Diese Klasse hat 2 Methoden, eine die mit der Klasse der Parameterkonfiguration arbeitet und die eine die den Sendevorgang schlussendlich ausführt.\\
Die Methode SendMail greift nun auf die Warteschlange aus Paketen zu und holt sich schritt für schritt ein Paket raus und ruft mit den Parametern jenes Paketes die SendMailReal Methode auf. SendMailReal sendet nun wirklich die E-Mail.

Man sieht in der SendMailReal Methode den Verbindungsaufbau zu den jeweiligen SMTP-Servern der Benutzer. Nachdem eine E-Mail versandt wurde bzw. nach gewünschten versuchen scheitert gesendet zu werden, wird die Verbindung zum Server beendet.

Im Log steht bei einem Fehlversuch der Grund dessen.
\begin{lstlisting}
