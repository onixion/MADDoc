...
\title{\textbf{MAD-Network Monitoring\\
...
\newpage Während der Entwicklungsphase, meinte Tomlinson, während er es seinen Kollegen zeigte, 
\begin{center}
...
\subsection{Aufbau}
Der allgemeine Aufbau einer E-Mail wird in zwei Teile aufgeteilt:
\begin{enumerate}
\item Header\\
Dieser beinhaltet Informationen über
\begin{itemize}
\item Absender
\item Empfänger und eventuelle zu sendenden Kopien
\item eventueller Betreff
\item eventueller Anhang
\end{itemize}
\item Body\\
...
\end{center}
\vspace{0.5cm}
Der Header ist sozusagen der Briefkuvert. Er enthält die für das Senden notwendigen Informationen. Im Gegensatz zu anderen Nachrichtensystemen ist es bei E-Mail möglich, eine Nachricht an mehrere Empfänger gleichzeitig zu senden.\\
Wie man im obigen Bild gut erkennen kann, kann man dabei sogar den Empfängern Stufen zuordnen, die dar wären:
\begin{itemize}
\item CC - Carbon Copy\\
Man bezeichnet sie auch oft als 'Courtesy Copy', was soviel bedeutet wie Höflichkeits-Kopie. Sie wird dazu verwendet, um Empfänger hinzuzufügen, welche nicht als Hauptempfänger, sondern als Empfänger gelten, welche nur Wissen sollten, dass diese E-Mail versendet wurde bzw. vielleicht auch einen Blick darauf werfen sollten.
\item BCC - Blind Carbon Copy\\
Diese Stufe wird verwendet, um Empfänger hinzuzufügen, über die keiner Bescheid wissen soll, wie z.B. bei Rundmails, um die Privatsphäre aller zu gewährleisten.
\end{itemize}  
Die Pfeile im obigen Bild zeigen schön, wer wen sehen kann.
\subsection{Senden}
Zum Senden einer E-Mail, wird ein schon ziemlich altes Protokoll verwendet, namens SMTP - Simple Mail Transfer Protocol. Es wird, dennoch Heute sehr viel verwendet. Es gibt nicht wirklich Alternativen zu SMTP, höchstens Weiterentwicklungen.
\subsubsection{SMTP}
Wie bereits gesagt, ist dies das zum Senden von E-Mails benötigte Protokoll.\\\\
Eigenschaften:\\
\begin{itemize}
\item Port:
\begin{itemize}
\item 25/TCP
\item 465/TCP\\
für SMTPS (SMTPSecure)
\item 587/TCP\\
für Mail-Clients (bzw. E-Mail Programm)
\end{itemize}
\item Netzwerklayer: Anwendung
\item RFC 5321 (Ursprünglich 821)
\end{itemize}
\paragraph{Allgemein - Sendevorgang\\\\}
Um den Sendevorgang erklären zu können müssen vorerst ein paar Begriffe näher erklärt werden.
\begin{itemize}
\item MUA - Mail User Agent\\
Der Mail User Agent bildet die Schnittstelle zwischen Benutzer und 'System' E-Mail. Also ist er von große Bedeutung. Es gibt unzählige Anbieter für einen solchen Agent. Genauer gesagt ist es eigentlich ein Programm, meist graphisch aufbereitet und bildet somit für den Benutzer eine komfortabele und ansprechende Schnittstelle um mit dem System zu agieren. Der Mail User Agent sorgt sowohl für das Senden also auch Empfangen einer E-Mail.\\\\
...
Der Mail Submission Agent bildet eine kleine Vorstufe, bevor es zum Mail Transfer Agent (im nächsten Punkt erklärt) geht. Ein Mail Submission Agent ist kein Muss, denn seine Aufgaben können auch vom Mail Transfer übernommen werden, jedoch gibt es ein paar Vorteile die für den Einsatz von Mail Submission Agents sprechen. Die dar wären:
\begin{itemize}
\item Der MSA kann sozusagen einen Vorfilter darstellen, denn falls der Benutzer eine E-Mail nicht richtig versendet hat (Datum vergessen, Empfänger vergessen,...) und sein MUA dies nicht abfängt, kann der MSA sofort eine Fehlermeldung schicken und die E-Mail kann dementsprechend nachbearbeitet werden, wohingegen wenn der MTA dies abfangen müsste, die Fehlermeldung den Benutzer, wenn überhaupt, nach Abbruch erreichen würde.
\item Die meisten MSAs verlangen eine Benutzerauthentifizierung, somit wird genau protokolliert wer, wem und wann eine E-Mail gesendet hat. Dies hilft MTAs insofern, dass Absender die Spam-Traffic (siehe Kapitel \ref{sssec:Spam} Seite \pageref{sssec:Spam}) verursachen, relativ schnell ausfindig gemacht werden können.
\end{itemize}
...
\item MX - Mail Exchanger\\
Nachdem der MTA des Absenders weiß wohin die E-Mail gesendet werden soll schickt er sie dem Mail Exchanger Server des Empfängers. Also ist der MX der erste Server des Empfängers, mit dem die E-Mail nun Kontakt hat.
\item MDA - Mail Delivery Agent\\
Dieser Agent sorgt für die Zuweisung der jeweiligen E-Mails zu den richtigen Benutzerkonten. Er ist jener Server, den der MUA des Empfängers nun anspricht, um die Mails abzurufen.
\end{itemize}
\begin{center}
\includegraphics[scale=0.45]{../docs/lyaton/graphics/SMTP-Uebertragung.png}\\
...
Der Sendevorgang startet beim MUA (Mail User Agent), welcher einfach ein Programm zum E-Mail versenden ist. Hier findet die Erste und Einzige Interaktion seitens des Absenders der E-Mail statt. Nachdem die E-Mail versendet wurde, 
...
\section{MAD-DB}
Bei der Datenbank handelt es sich um eine relationale Datenbank, welche in C\# geschrieben wurde. Die Implementierung der SQLite- Datenbank erfolgte durch das Einbinden der "{}sqlite3.dll"{}.
\subsection{Erklärung}
Die Datenbank besteht aus zwölf verschiedenen Tabellen. Die Namen der Tabellen sagen aus, welche Inhalte in ihnen gespeicher werden. Es wurden zwölf Tabellen ersellt um das System deterministisch zu halten, damit keine weiteren Veränderung in der Datenbank vorgenommen werden müssen.\\
...
\begin{enumerate}
\item ID als primary key vom Typ Integer
\item MEMO1 vom Typ Text, hier werden kurze Notizen vom Administrator zu besimmten Geräten gespeichert
\item MEMO2 vom Typ Text, hier werden lange Notizen vom Administragor gespeichert
\end{enumerate}
\subsubsection{JobTable}
Diese Tabelle besteht aus 14 Feldern:
\begin{enumerate}
\item ID als primary key vom Integer
...
Diese Felder werden verwendet, um alle Daten aus den anderen Tabellen zu den dazugehörigen Daten aus anderen Tabellen zuzuordnen und diese später leichter auszugeben. Diese Verknüpfung erfolgt über eine Join-Funktione. Die Dynamik und Effizienz der Datenbank wird auf diese Weise gesteigert.
