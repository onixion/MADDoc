...
\lfoot{Manpreet Singh}
Nach einigen Startschwierigkeiten beim Entscheiden, wer nun diesen teil programmiert, ist schlussendlich mir die Aufgabe zuteilgeworden.\\\\
Dieser Teil war besonders Herausfordern, wie ich finde. Das Problem liegt darin alle Geschmäcker zu treffen. Der Eine legt wert auf Effizienz wohingegen der Andere auf Optik.\\\\
Nach 3 Versuchen klappte es eine GUI zu programmieren, die den Zweck erfüllt und optisch auch was hermacht.\\
...
\textbf{Nodes-Fenster}
\end{center}
Die Arbeit and dieser Version wurde bald eingestellt, da dies eher ein 'Testobjekt' war. Allerdings habe ich wertvolle Erfahrung gemacht und hatten meinen ersten Kontakt mit Visual C\#
\subsubsection{Version 2}
\begin{center}
\includegraphics[scale=0.65]{../docs/lyaton/graphics/GUI_v2_config.png}\\
\textbf{Konfigurationsfenster}
\end{center}
Leider erst viel zu spät wurde mir bewusst, dass ich mich in etwas viel zu aufwendiges verstrickt hatte. Meine Herangehensweise war vollkommen falsch. Ich habe viel zu Groß gedacht, obwohl man bei genau solchen Projekten, wie einer graphischen Oberfläche, wo einem unendlich viele Möglichkeiten geboten werden, eigentlich ganz klein denken muss. Jedoch habe ich erneut einiges gelernt. Der Umgang mit Visual C\# fiel mir immer einfacher.
\subsubsection{Verion 3 - Final Version}
\begin{center}
\includegraphics[scale=0.65]{../docs/lyaton/graphics/GUI_v3_demo.png}\\
\textbf{Hauptfenster}
\end{center}
Diese Version ist knapp und knackig. Sie erfüllt die gewünschten Anforderungen und sieht dabei, für die meisten zumindest, gut aus. Ich wollt hier noch ein paar Features, wie z.B. Graphen,  implementieren, dies war jedoch, zeitlich nicht mehr möglich.\\

Alles in allem kostete mich das Programmieren der GUI viele Stunden und vor allem viele Nerven. Es war aber auf jeden Fall eine wertvolle Erfahrung, welche ich auch ihn meinem weiteren Leben gut gebrauchen kann.
\newpage
...
\lfoot{Manpreet Singh}
Dieser Teile war mir ein großes Anliegen. Für mich war es wichtig, eine Möglichkeit zu bieten, mit der man den User benachrichtigen kann. Nach kurzer Diskussion mit meinen Kollegen entschieden wir uns für die E-Mail zum benachrichtigen.\\

Dies war ebenfalls mein erster Kontakt mit C\#, deshalb braucht ich anfänglich etwas länger. Als ich die Sprache ein wenig beherrschte, war die erste Version der Benachrichtigung schnell geschrieben. \\

Nach ewigem testen, stellte sich heraus, dass das, das ich programmiert hatte leider nicht Effizienzideologie unseres Projektes entsprach, denn jedes mal als eine E-Mail versendet wurde, musst das Programm im schlimmsten Fall 7 Sekunden warten, bis es dann weitermachen konnte. Also machte ich mich wieder an die Arbeit. Doch nach kurzer Absprache mit Teamkollegen Alin Porcic, war schnell klar das die einzige Lösung hier Threads bieten (siehe Kapitel \ref{ssec:Threads} Seite \pageref{ssec:Threads}).\\

Nach einigen weiteren Erweiterungen wie z.B. des Que Systemes, welches dem Programm erlaubt unzählige E-Mails, unabhängig von Wartezeiten, dem Notification System zu übergeben. Dies ist möglich durch das Verwenden von Ques (Warteschlangen). Aber auch andere Features wie optionale E-Mail Felder habe ich implementiert. Optionale E-Mail Felder sind beispielsweise Dateianhang, CC, BCC, Priorität, ...\\

Alles in allem war dies eine eher einfache Aufgabe.
...
Von hieran ist es am Sinnvollsten, eine Scan durchzuführen. Dieser Scan listet dann alle gefunden Geräte im Netzwerk auf.
\begin{center}
\includegraphics[scale=0.7]{../docs/lyaton/graphics/GUI_v3_scan.png}\\
\textbf{Scanfenster}
\end{center}
Dieses Scanfenster benötigt nun ein Paar Informationen:
\begin{itemize}
...
\item Network\\
Hier wird nach dem Netzwerk gefragt.
\item Subnet\\
...
\begin{itemize}
\item Network Device\\
Hier wird nach der Netzwerkarte gefragt, wobei man nur aus den verfügbaren Netzwerkkarten auswählen kann. Falls man nichts auswählt wird automatisch die in der Konfiguration vermerkte Netzwerkkarte verwendet.
\item Single Scan\\
Hier wird gefragt ob man nur einen Scan machen möchten, anstatt einen sich immer wiederholenden.
\end{itemize}
\end{itemize}
Nachdem der Scan gestartet wurde verschwindet das Scanfenster wieder und man ist zurück im Hauptfenster. Nach kurzer Zeit (Standardmäßig 30 Sekunden, können jedoch auch mehr sein, da dies einstellbar ist.) sollte man auf dem Reload-Button im linken unteren Eck klicken. \\Folgendes Fenster erscheint dann:
\begin{center}
\includegraphics[scale=0.7]{../docs/lyaton/graphics/GUI_v3_reload.png}\\
\textbf{Hauptfenster nach abgeschlossenem Scan}
\end{center}
Man kann nun auf die einzelnen Nodes im linken Bereich klicken und mehr Informationen darüber im rechten Bereich erhalten.
\begin{center}
\includegraphics[scale=0.7]{../docs/lyaton/graphics/GUI_v3_nodeclick.png}\\
\textbf{Hauptfenster bei geöffneter Node}
\end{center}
Um den Node aktivieren zu können bzw. einem Node jobs zuweisen zu können, muss man die CLI starten. Dafür ist im oberen rechten Eck ein Button namens CLI.\\Nach dem Klicken passiert folgendes:
\begin{center}
\includegraphics[scale=0.7]{../docs/lyaton/graphics/GUI_v3_cli.png}\\
\textbf{CLI}
\end{center}
Dort kann man wie im Punkt CLI beschrieben, seine Befehle ausführen (siehe Kapitel \ref{chap:CLI_UM} Seite \pageref{chap:CLI_UM}). Nachdem man die gewünschten Jobs den Nodes zugeordnet hat und die Nodes, die Jobs und den Scheduler gestartet hat, kann man zurück zur graphischen Oberfläche.\\\\
Wenn man dort nun erneut auf den Reload-Button klickt sieht man:
\begin{center}
\includegraphics[scale=0.7]{../docs/lyaton/graphics/GUI_v3_jobclick.png}\\
\textbf{Hauptfenster bei geöffneter Node}
\end{center}
In diesem Fenstern kann man nun wunderbar alle Nodes und die jeweiligen Jobs ansehen.\\\\
Die letzte Funktion der graphischen Oberfläche ist das Informationsfenster. Dieses erreicht man indem man im rechten oberen Eck auf den Info-Button klickt.
\begin{center}
\includegraphics[scale=0.7]{../docs/lyaton/graphics/GUI_v3_info.png}\\
\textbf{Infofenster}
\end{center}
\part{Zeiteinteilung}
